...
function getPolynom(...args) {
  const polynomArgs = args.reverse();
  return (x) => polynomArgs.reduce((sum, arg, index) => sum + arg * x ** index, 0);
}
...
function memoize(func) {
  const cache = {};
  return () => {
    const key = JSON.stringify(func.toString());
    if (!cache[key]) {
      cache[key] = func.call();
    }
    return cache[key];
...
function logger(func, logFunc) {
  return (...args) => {
    const funcSignature = `${func.name}(${args.map((arg) => JSON.stringify(arg))})`;
...
function getIdGeneratorFunction(startFrom) {
  ...
  let cur = startFrom;
  return () => {
    cur = invocationCount ? cur + 1 : startFrom;
    invocationCount += 1;
    return cur;
  };
}
