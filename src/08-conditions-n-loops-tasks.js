...
function isTriangle(a, b, c) {
  const triangleSidesArr = Array.of(a, b, c);
  triangleSidesArr.sort((first, second) => first - second);
  return triangleSidesArr[0] + triangleSidesArr[1] > triangleSidesArr[2];
}
...
function doRectanglesOverlap(rect1, rect2) {
  ...
  if (rect2bottom < rect1.top || rect2.top > rect1bottom) return false;
  if (rect2right < rect1.left || rect2.left > rect1right) return false;
...
function isInsideCircle(circle, point) {
  ...
  return Math.sqrt(distance) < circle.radius;
}
...
function findFirstSingleChar(str) {
  ...
  for (let chIndex = 0; chIndex < charArr.length; chIndex += 1) {
    const char = charArr[chIndex];
    if (charArr.indexOf(char) === charArr.lastIndexOf(char)) return char;
  }
  return null;
}
...
function isCreditCardNumber(ccn) {
  const luhnSum = ccn
    ...
    .map((digit, index) => {
      if (index % 2) {
        const doubleDigit = parseFloat(digit) * 2;
        if (doubleDigit > 9) {
          return (doubleDigit % 10) + Math.floor(doubleDigit / 10);
        }
        return doubleDigit;
      }
...
function getDigitalRoot(num) {
  const digitsSum = [...num.toString()].reduce(
    (acc, cur) => parseFloat(acc) + parseFloat(cur),
  );
  return digitsSum > 9 ? getDigitalRoot(digitsSum) : digitsSum;
}
...
function getCommonDirectoryPath(pathes) {
  const dirSeparator = new RegExp('(.*?/)');
  let commonPath;

  for (let i = 0; i < pathes.length - 1; i += 1) {
    const firstPath = pathes[i].split(dirSeparator);
    const secondPath = pathes[i + 1].split(dirSeparator);
    const curCommonPath = firstPath.filter((value) => secondPath.includes(value));
    if (commonPath && curCommonPath.length > commonPath.length) return commonPath.join('');
    commonPath = curCommonPath;
  }
